    ...
    Statement = [
      {
        ...
        Effect   = "Allow"
        Resource = "arn:aws:dynamodb:*:*:table/${aws_dynamodb_table.connection.name}"
      },
    ]
  })
}
    ...
    Statement = [
      {
        ...
        Effect   = "Allow"
        Resource = "${aws_apigatewayv2_api.websocket_gw.execution_arn}/${aws_apigatewayv2_stage.stage.name}/POST/@connections/{connectionId}"
      },
    ]
  })
}